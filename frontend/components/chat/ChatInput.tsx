...
export function ChatInput({
  ...
  const handleSend = () => {
    ...
    if (isManualMode && onGenerateManual) {
      ...
      {!isRecording && audioChunks.length > 0 && (
        <div className="flex items-center gap-2 mb-2 p-3 bg-muted/50 rounded-lg text-xs sm:text-sm text-orange-500">
          ...
          <button
            ...
            className="hover:text-red-500 ml-auto min-h-[32px] min-w-[32px] flex items-center justify-center"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="relative group">
        ...
        <div className="relative bg-card border border-border rounded-xl sm:rounded-2xl shadow-lg ring-1 ring-white/5 transition-all focus-within:ring-orange-500/50">
          ...
          <textarea
            ...
            disabled={isLoading || isRecording || audioChunks.length > 0}
          ...
          <div className="flex items-center justify-between px-3 sm:px-4 pb-3 sm:pb-4 gap-2 z-20 relative">
            {/* Left Actions - Buttons */}
            <div className="flex items-center gap-2">
              <button
                onClick={handleAttachClick}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/50 hover:bg-muted text-xs sm:text-sm font-medium text-muted-foreground hover:text-foreground transition-all duration-200 border border-transparent hover:border-border"
                title="Upload Image"
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {showSendButton ? (
                ...
                <button
                  onClick={handleSend}
                  disabled={isLoading}
                  className={`flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl transition-all active:scale-95 shadow-lg text-white disabled:opacity-50 ${
                    isManualMode
                      ? "bg-gradient-to-tr from-blue-500 to-blue-600 shadow-blue-500/25 hover:shadow-blue-500/40 hover:scale-105"
                      : "bg-gradient-to-tr from-green-500 to-green-600 shadow-green-500/25 hover:shadow-green-500/40 hover:scale-105"
                  }`}
                  title={isManualMode ? "Generate Manual" : "Send Message"}
                >
                  {isManualMode ? (
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl transition-all active:scale-95 shadow-lg ${
                    isRecording
